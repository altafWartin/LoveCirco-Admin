...
import { useNavigate, useParams } from "react-router-dom"; // Import useNavigate from React Router

const UpdatePlan = () => { const apiUrl = process.env.REACT_APP_API_URL;
    ...
    // Use planeId in your component logic
    // For demonstration, let's log it
    console.log("Plane IDdd:", planId);
    useEffect(() => {
        const fetchPlanDetails = async () => {
            console.log("Plane IDdddddddd:", planId);

            try {
                const response = await fetch(`${apiUrl}/api/getMembershipPlan/${planId}`);
                ...
                setPrice(data.data.price.toString()); // Convert to string if needed
                setPriceWithoutDiscount(data.data.priceWithoutDiscount.toString()); // Convert to string if needed
                setDuration(data.data.duration.toString()); // Convert to string if needed
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/updateMembershipPlan/${planId}`, {
                method: "PUT",
                headers: {
